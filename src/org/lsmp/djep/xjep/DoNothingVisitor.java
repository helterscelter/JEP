...
/**
 * A Visitor which visits each node and returns the top node.
...
public abstract class DoNothingVisitor implements ParserVisitor {
	...
	public Node[] acceptChildrenAsArray(Node node,Object data)  throws ParseException
	{
		int n = node.jjtGetNumChildren();
		Node children[] = new Node[n];
		for(int i=0;i<n;++i)
			children[i]= (Node) nodeAccept(node.jjtGetChild(i),data);
		return children;
	}
